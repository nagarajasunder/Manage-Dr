...
class DoctorRepositoryImpl @Inject constructor(
    ...
    override suspend fun addNewDoctor(mdrDoctor: MdrDoctor) = externalScope.launch(dispatcher) {
        dao.insertNewDoctor(mdrDoctor)
    }.join()

    override fun getDoctorById(doctorId: Int) = dao.getDoctorById(doctorId)
    override suspend fun updateDoctor(doctor: MdrDoctor) = dao.updateDoctorAndTransactions(doctor)
    override suspend fun addNewCity(newCity: MdrCity) = cityDao.insertNewCity(newCity)
    override fun getAllCities() : Flow<List<MdrCity>> = cityDao.getAllCities()
